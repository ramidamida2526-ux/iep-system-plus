...
def upload_profile():
    ...
    if file:
        ...
        # ตั้งค่าโฟลเดอร์เก็บรูปภาพ
        upload_dir = os.path.join('static', 'uploads', 'profile_pics')
        os.makedirs(upload_dir, exist_ok=True)
        
        # บันทึกไฟล์โดยใช้ ID ผู้ใช้ตั้งชื่อป้องกันชื่อซ้ำ
        filename = secure_filename(f"user_{current_user.id}_{file.filename}")
        file.save(os.path.join(upload_dir, filename))
        
        # บันทึกชื่อไฟล์ลงฐานข้อมูลในตารางผู้ใช้ปัจจุบัน
...
def create_initial_users():
    if User.query.first() is None:
        users = [
            ...
            User(username='teacher1', password_hash=generate_password_hash('1234'), fullname='ครูสมศรี (ครูผู้สอน)'),
            User(username='boss', password_hash=generate_password_hash('1234'), fullname='ผอ. สมศักดิ์ (ผู้บริหาร)')
        ]
